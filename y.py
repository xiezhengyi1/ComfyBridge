...
try:
...
except Exception:  # compatibility with older transformers without QuantizedCacheConfig
    from transformers.cache_utils import (
        Cache,
        DynamicCache,
        EncoderDecoderCache,
    ...
    class QuantizedCacheConfig:  # minimal shim
        def __init__(self, *args, **kwargs):
            pass
    class OffloadedCache:  # minimal shim (removed in transformers>=4.56)
        def __init__(self, *args, **kwargs):
            pass
...
try:
    ...
    AssistedCandidateGenerator = object  # type: ignore
    AssistedCandidateGeneratorDifferentTokenizers = object  # type: ignore
    CandidateGenerator = object  # type: ignore
    PromptLookupCandidateGenerator = object  # type: ignore

    def _crop_past_key_values(past_key_values, *args, **kwargs):  # type: ignore
        return past_key_values

    def _prepare_attention_mask(*args, **kwargs):  # type: ignore
        return kwargs.get("attention_mask")

    def _prepare_token_type_ids(*args, **kwargs):  # type: ignore
...
try:
    ...
    # Compatibility shims for older transformers
    from transformers.generation.configuration_utils import (
        GenerationConfig,
        GenerationMode,
    )
    NEED_SETUP_CACHE_CLASSES_MAPPING = set()  # type: ignore
...
def isin_mps_friendly(elements, test_elements, **kwargs):
    return torch.isin(elements, test_elements, **kwargs)
